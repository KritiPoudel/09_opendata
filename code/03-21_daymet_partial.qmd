...
library(daymetr) # to pull weather data from daymet
```
...
```{r unique years}
# length(unique(field$year)) done by myself
...
```{r}
daymet_extracted_unnest <- daymet_extracted %>% 
  unnest(weather) %>% 
  pivot_wider(names_from = measurement,
              values_from = value) %>% 
  janitor::clean_names()
...
Let's do that below:  
```{r}
#write.csv(daymet_extracted_unnest,"../data/datafor_five_sites.csv")
```
...
#write.csv(daymet_all_unnest,"../data/daymet_all_unnest.csv")
```
