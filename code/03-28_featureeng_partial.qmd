...
```{r}
fieldweather <- read.csv("../data/daymet_all_unnest.csv")
...
fe_month_sum_wide <- fe_month_sum %>% 
  pivot_longer(cols = dayl.s_mean:prcp.mm_sum) %>% 
...
ggplot(data = fe_month_sum, aes(x = tmax.c_mean,
                                y = month_abb,
                                fill = stat(x)))+
  geom_density_ridges_gradient(scale = 3,
                               rel_min_height = 0.01)+ #rel_min_ht cuts the tail
